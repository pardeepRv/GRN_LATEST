...
let DBGrnPurchaseOrderDataHelper = {
          ...
          if (
            ...
            DB.write(() => {
              (dbPurchaseOrder.order_number = po.order_number),
                ...
                (dbPurchaseOrder.quantity_ordered =
                  po && po.quantity_ordered != null ? po.quantity_ordered : 0),
            ...
            if (
              (dbPurchaseOrder.submitStatus == 'processing',
              dbPurchaseOrder.quantity_available_to_receive !=
                po.quantity_available_to_receive || duration > 300000)
            ) {
              if (dbPurchaseOrder.quantity_received != 0) {
                console.log('PROCESSING');
                console.log(
                  'print local quantity: ',
                  dbPurchaseOrder.quantity_received,
                );
              ...
              DB.write(() => {
                (dbPurchaseOrder.submitStatus = ''),
                  ...
                  (dbPurchaseOrder.quantity_available_to_receive =
                    po.quantity_available_to_receive);
  ...
  async updatePOStatus(
    ...
    if (distribution_number && order_line_number) {
      var dbExistingPurchaseOrders = DB.objects('PurchaseOrder').filtered(
        'order_number = $0 AND distribution_number == $1 AND order_line_number == $2',
        order_number,
        distribution_number,
        order_line_number,
      );
  ...
  async getPurchaseOrders() {
    const dbPurchaseOrders = DB.objects('PurchaseOrder').filtered(
      `TRUEPREDICATE SORT(order_number DESC, order_line_number ASC, distribution_number ASC)`,
    );
  ...
  async getPOByOrderNumber(order_number) {
    ...
    console.log(
      dbPurchaseOrders,
      'dbPurchaseOrdersdbPurchaseOrdersdbPurchaseOrders',
    );
    return dbPurchaseOrders;
  },

  async getCreateReceipts(order_number) {
    const dbCreateReceipts = DB.objects('CreateReceipt').filtered(
      'order_number == $0',
      order_number,
    );
  ...
  async updateCreateReceiptStatus(
    ...
    let dbUpdateReceipt = DB.objects('CreateReceipt').filtered(
      'order_number = $0 AND distribution_number == $1 AND order_line_number == $2',
      order_number,
      distribution_number,
      order_line_number,
  ...
  async deleteCreateReceipts(
    order_number,
    distribution_number,
    order_line_number,
  ) {
    let dbCreateReceipts = DB.objects('CreateReceipt').filtered(
      'order_number = $0 AND distribution_number == $1 AND order_line_number == $2',
      order_number,
      distribution_number,
      order_line_number,
